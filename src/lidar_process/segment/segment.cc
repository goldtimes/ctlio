...
int PCSeg::DoSeg(int *pLabel1, float *fPoints1, int pointNum) {
    ...
    if (this->corPoints != nullptr) free(this->corPoints);
    ...
    for (int ii = 10; ii < 256; ii++) {
        ...
        if (pnum > 0) {
            ...
            if (cf.cls < 1) {
                ...
                for (int jj = 0; jj < 8; jj++) this->pOBBs[clabel * 8 + jj] = cf.obb[jj];
...
int GetNeiborPCA(SNeiborPCA &npca, pcl::PointCloud<pcl::PointXYZ>::Ptr cloud, pcl::KdTreeFLANN<pcl::PointXYZ> kdtree,
...
int SegBG(int *pLabel, pcl::PointCloud<pcl::PointXYZ>::Ptr cloud, pcl::KdTreeFLANN<pcl::PointXYZ> &kdtree,
          float fSearchRadius) {
...
SClusterFeature FindACluster(int *pLabel, int seedId, int labelId, pcl::PointCloud<pcl::PointXYZ>::Ptr cloud,
...
int SegObjects(int *pLabel, pcl::PointCloud<pcl::PointXYZ>::Ptr cloud, pcl::KdTreeFLANN<pcl::PointXYZ> &kdtree,
               float fSearchRadius) {
    int pnum = cloud->points.size();
...
int CompleteObjects(int *pLabel, pcl::PointCloud<pcl::PointXYZ>::Ptr cloud, pcl::KdTreeFLANN<pcl::PointXYZ> &kdtree,
                    float fSearchRadius) {
    int pnum = cloud->points.size();
...
int FreeSeg(float *fPoints, int *pLabel, int pointNum) {
    ...
    if (pFreeDis != nullptr) free(pFreeDis);
}
...
int GndSeg(int *pLabel, float *fPoints, int pointNum, float fSearchRadius) {
    ...
    for (int pid = 0; pid < pointNum; pid++) {
        if (pLabel[pid] == 1) {
            ...
            {
                pLabel[pid] = 0;
            } else if (fPoints[pid * 4] * fPoints[pid * 4] + fPoints[pid * 4 + 1] * fPoints[pid * 4 + 1] <
