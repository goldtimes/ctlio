...
lidarodom::lidarodom(/* args */) {
    ...
    current_state = new state(Eigen::Quaterniond::Identity(), Eigen::Vector3d::Zero(), Eigen::Vector3d::Zero(),
...
bool lidarodom::init(const std::string &config_yaml) {
    ...
    StaticImuInit::InitOptions imu_init_options;
    imu_init_options.use_odom = false;  // 本节数据不需要轮速计
    ...
    LidarPlaneNormFactor::q_il = TIL_.rotationMatrix();
    // CTLidarPlaneNormFactor::t_il = t_imu_lidar;
    // CTLidarPlaneNormFactor::q_il = TIL_.rotationMatrix();
...
void lidarodom::run() {
    while (true) {
        ...
        cond.wait(lk, [&] {
            ...
            // std::cout << "measure size:-------------" << measurements.size() << std::endl;
        ...
        for (auto &m : measurements) {
            Timer::Evaluate([&]() { ProcessMeasurements(m); }, "processMeasurement");

            {
                ...
                if (real_time - prev_real_time > std::chrono::seconds(5)) {
                    ...
                    auto delta_real =
                        std::chrono::duration_cast<std::chrono::milliseconds>(real_time - prev_real_time).count() *
                        0.001;
...
void lidarodom::ProcessMeasurements(MeasureGroup &meas) {
    ...
    // std::cout << ANSI_COLOR_GREEN << "============== process frame: " << index_frame << ANSI_COLOR_RESET <<
    // std::endl;
    imu_states_.clear();  //   need clear here
    // std::cout << "begin time:" << meas.lidar_begin_time_ << ",end_time:" << meas.lidar_end_time_ << std::endl;
    // 利用IMU数据进行状态预测
    ...
    cloudFrame *p_frame;
    Timer::Evaluate(
        [&]() { p_frame = buildFrame(const_surf, current_state, meas.lidar_begin_time_, meas.lidar_end_time_); },
        "build frame");

    //   lio
    Timer::Evaluate([&]() { poseEstimation(p_frame); }, "poseEstimate");

    //   观测
    SE3 pose_of_lo_ = SE3(current_state->rotation, current_state->translation);
    // std::cout << "obs: " << current_state->translation.transpose() << ", " << current_state->rotation.transpose()
    //           << std::endl;
    SE3 pred_pose = eskf_.GetNominalSE3();
    // std::cout << "pred: " << pred_pose.translation().transpose() << ", " << pred_pose.so3().log().transpose()
    //           << std::endl;
    Timer::Evaluate([&]() { eskf_.ObserveSE3(pose_of_lo_, 1e-2, 1e-2); }, "eskf_obs");
    ...
    Timer::Evaluate(
        [&]() {
            ...
            SE3 pred_pose = eskf_.GetNominalSE3();
            Eigen::Vector3d vel_world = eskf_.GetNomialVel();
...
void lidarodom::poseEstimation(cloudFrame *p_frame) {
    ...
    if (index_frame > 1) {
        Timer::Evaluate([&]() { optimize(p_frame); }, "optimize");
    }

    bool add_points = true;
    if (add_points) {  //   update map here
        Timer::Evaluate([&]() { map_incremental(p_frame); }, "map update");
    }

    Timer::Evaluate([&]() { lasermap_fov_segment(); }, "fov segment");
}
...
void lidarodom::optimize(cloudFrame *p_frame) {
    ...
    // std::cout << "origin size:" << num_size << ", keypoints:" << surf_keypoints.size() << std::endl;
    auto transformKeypoints = [&](std::vector<point3D> &point_frame) {
        Eigen::Matrix3d R;
        Eigen::Vector3d t;
        for (auto &keypoint : point_frame) {
            if (options_.point_to_plane_with_distortion || options_.icpmodel == IcpModel::CT_POINT_TO_PLANE) {
    ...
    for (int iter(0); iter < options_.max_num_iteration; iter++) {
        ...
        for (auto &e : surfFactor) {
            ...
            switch (options_.icpmodel) {
                case IcpModel::CT_POINT_TO_PLANE:
                    problem.AddResidualBlock(e, loss_function, &begin_t.x(), &begin_quat.x(), &end_t.x(),
                                             &end_quat.x());
                    break;
                case IcpModel::POINT_TO_PLANE:
        ...
        //                     previous_translation, sqrt(surf_num * options_.beta_location_consistency *
        //                     laser_point_cov));
        // #else
        //                 auto *cost_location_consistency = LocationConsistencyFunctor::Create(
        //                     previous_translation, sqrt(surf_num * options_.beta_location_consistency));
        // #endif
        ...
        //                     previous_orientation, sqrt(surf_num * options_.beta_orientation_consistency *
        //                     laser_point_cov));
        // #else
        //                 auto *cost_rotation_consistency = OrientationConsistencyFunctor::Create(
        //                     previous_orientation, sqrt(surf_num * options_.beta_orientation_consistency));
        // #endif
        ...
        //                     new SmallVelocityFactor(sqrt(surf_num * options_.beta_small_velocity * laser_point_cov));
        // #else
        //                 auto *cost_small_velocity = SmallVelocityFunctor::Create(sqrt(surf_num *
        ...
        // {
        //      VelocityConsistencyFactor2 *cost_velocity_consistency =
        //          new VelocityConsistencyFactor2(previous_velocity, sqrt(surf_num *
        //          options_.beta_constant_velocity * laser_point_cov));
...
double lidarodom::checkLocalizability(std::vector<Eigen::Vector3d> planeNormals) {
    ...
    if (planeNormals.size() > 10) {
        ...
        if (svd.singularValues().z() < 3.5)
            std::cout << "Low convincing result -> singular values:" << svd.singularValues().x() << ", "
                      << svd.singularValues().y() << ", " << svd.singularValues().z() << std::endl;
        return svd.singularValues().z();
    } else {
        std::cout << "Too few normal vector received -> " << planeNormals.size() << std::endl;
        return -1;
    }
}
...
void lidarodom::addSurfCostFactor(std::vector<ceres::CostFunction *> &surf, std::vector<Eigen::Vector3d> &normals,
    ...
    for (int k = 0; k < num; k++) {
        ...
        if (vector_neighbors.size() < options_.min_number_neighbors) continue;
        ...
        for (int i(0); i < options_.num_closest_neighbors; ++i) {
            ...
            // std::cout << "point_to_plane_dist:" << point_to_plane_dist << std::endl;
            if (point_to_plane_dist < options_.max_dist_to_plane_icp) {
                ...
                switch (options_.icpmodel) {
                    case IcpModel::CT_POINT_TO_PLANE: {
                        ...
                        //                             keypoints[k].raw_point, norm_vector, norm_offset,
                        //                             keypoints[k].alpha_time, weight);
                        // #else
                        //                         auto *cost_function = CTPointToPlaneFunctor::Create(
                        //                             vector_neighbors[0], keypoints[k].raw_point, norm_vector,
                        //                             keypoints[k].alpha_time, weight);
                        // #endif
                        // surf.push_back(cost_function);
                        // problem.AddResidualBlock(cost_function, loss_function, &begin_t.x(), &begin_quat.x(),
                        // &end_t.x(), &end_quat.x());
                        break;
                    }
                    case IcpModel::POINT_TO_PLANE: {
...
#else
                        auto *cost_function =
                            PointToPlaneFunctor::Create(vector_neighbors[0], point_end, norm_vector, weight);
#endif
                        surf.push_back(cost_function);
        ...
        for (short kyy = ky - nb_voxels_visited; kyy < ky + nb_voxels_visited + 1; ++kyy) {
            for (short kzz = kz - nb_voxels_visited; kzz < kz + nb_voxels_visited + 1; ++kzz) {
                ...
                if (search != map.end()) {
                    ...
                    if (voxel_block.GetNumPoints() < threshold_voxel_capacity) continue;
...
void lidarodom::addPointToPcl(pcl::PointCloud<pcl::PointXYZI>::Ptr pcl_points, const Eigen::Vector3d &point,
                              const double &intensity, cloudFrame *p_frame) {
...
void lidarodom::map_incremental(cloudFrame *p_frame, int min_num_points) {
    ...
    for (const auto &point : p_frame->point_surf) {
        addPointToMap(voxel_map, point.point, point.intensity, options_.size_voxel_map,
                      options_.max_num_points_in_voxel, options_.min_distance_points, min_num_points, p_frame);
    }
    if (index_frame < 2) {
        std::cout << "init build map size:" << voxel_map.size() << std::endl;
    }
    {
        std::string laser_topic = "laser";
...
void lidarodom::lasermap_fov_segment() {
    ...
    for (auto &vox : voxels_to_erase) voxel_map.erase(vox);
    std::vector<voxel>().swap(voxels_to_erase);
}

cloudFrame *lidarodom::buildFrame(std::vector<point3D> &const_surf, state *cur_state, double timestamp_begin,
    ...
    if (options_.motion_compensation == CONSTANT_VELOCITY) Undistort(frame_surf);

    for (auto &point_temp : frame_surf)
        transformPoint(options_.motion_compensation, point_temp, cur_state->rotation_begin, cur_state->rotation,
                       cur_state->translation_begin, cur_state->translation, R_imu_lidar, t_imu_lidar);
...
std::vector<MeasureGroup> lidarodom::getMeasureMents() {
    std::vector<MeasureGroup> measurements;
    while (true) {
        if (imu_buffer_.empty()) return measurements;

        if (lidar_buffer_.empty()) return measurements;

        if (imu_buffer_.back()->timestamp_ - time_curr < delay_time_) return measurements;
        ...
        if (!imu_buffer_.empty()) meas.imu_.push_back(imu_buffer_.front());  //   added for Interp
...
void lidarodom::Predict() {
    ...
    // std::cout << "imu begin time:" << measures_.imu_.front()->timestamp_
    //   << ",imu end time:" << measures_.imu_.back()->timestamp_ << std::endl;
    for (auto &imu : measures_.imu_) {
        ...
        if (imu->timestamp_ <= time_current) {
            ...
            Eigen::Vector3d acc_temp = w1 * last_imu_->acc_ + w2 * imu->acc_;
            Eigen::Vector3d gyr_temp = w1 * last_imu_->gyro_ + w2 * imu->gyro_;
            IMUPtr imu_temp = std::make_shared<IMU>(time_current, gyr_temp, acc_temp);
            eskf_.Predict(*imu_temp);
            imu_states_.emplace_back(eskf_.GetNominalState());
            last_imu_ = imu_temp;
        }
    }
    // std::cout << "predict state:" << imu_states_.back() << std::endl;
}
...
void lidarodom::Undistort(std::vector<point3D> &points) {
    ...
    for (auto &pt : points) {
        ...
        PoseInterp<NavState>(pt.timestamp, imu_states_, [](const NavState &s) { return s.timestamp_; },
...
void lidarodom::TryInitIMU() {
    ...
    if (imu_init_.InitSuccess()) {
        ...
        std::cout << "init state:" << eskf_.GetNominalState() << std::endl;
        std::cout << "IMU初始化成功" << std::endl;
    }
}
