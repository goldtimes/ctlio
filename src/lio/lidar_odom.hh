...
class lidarodom {
   public:
    ...
    void setFunc(std::function<bool(std::string &topic_name, double time1, double time2)> &fun) {
        pub_data_to_ros = fun;
    }
   ...
   private:
    ...
    cloudFrame *buildFrame(std::vector<point3D> &const_surf, state *cur_state, double timestamp_begin,
    ...
    void addPointToPcl(pcl::PointCloud<pcl::PointXYZI>::Ptr pcl_points, const Eigen::Vector3d &point,
   ...
   private:
    ...
    /// @brief 滤波器
    ESKFD eskf_;
    std::vector<NavState> imu_states_;  // ESKF预测期间的状态
